...
export function StatusBar() {
  ...
  const cells = doc.grid.type === 'none'
    ? null
    : `${Math.round(doc.width / doc.grid.size)}×${Math.round(doc.height / doc.grid.size)} cells`;
  ...
  return (
    <div className="statusbar">
      ...
      {editor.selection.objectIds.length > 0 && (
        <>
          <span>{editor.selection.objectIds.length} selected</span>
          <span className="sep" />
        </>
      )}
      ...
      <span className="sep" />
      <span>{walls} walls · {lights} lights</span>
      <span className="sep" />
      <span>{editor.history.depth} undo steps</span>
    </div>
  );
}
